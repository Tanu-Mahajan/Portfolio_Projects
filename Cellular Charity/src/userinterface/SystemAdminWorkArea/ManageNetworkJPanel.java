...
public class ManageNetworkJPanel extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        networkJTable.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        ...
        deleteButton1.setBackground(new java.awt.Color(204, 255, 204));
        deleteButton1.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        deleteButton1.setForeground(new java.awt.Color(51, 0, 51));
    ...
    private void submitJButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_submitJButtonActionPerformed
        ...
        if (!nameFlag){
            ...
            network.getEnterpriseDirectory().createAndAddEnterprise("Main Hub",Enterprise.EnterpriseType.CentralHub);
            network.getEnterpriseDirectory().createAndAddEnterprise("Inspection Hub",Enterprise.EnterpriseType.InspectionHub);
            network.getEnterpriseDirectory().createAndAddEnterprise("Recycling Hub",Enterprise.EnterpriseType.ScrapHub);
            network.getEnterpriseDirectory().createAndAddEnterprise("Logistics Nexus",Enterprise.EnterpriseType.LogisticsNexus);

            for(Enterprise enterprise: network.getEnterpriseDirectory().getEnterpriseList())  
            {
                if(enterprise instanceof CentralHubEnterprise)
                {
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.RetailChain);
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.Donor);
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.HeadOffice);
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.Inventory);
                }
                else if(enterprise instanceof InspectionHubEnterprise)
                {
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.QualityAssuranceCenter);
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.Technician);
                } 
                else if(enterprise instanceof LogisticsNexusEnterprise)
                {
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.Logistics);
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.Driver);
                } 
                else if(enterprise instanceof ScrapHubEnterprise)
                {
                   enterprise.getOrganizationDirectory().createOrganization(Organization.Type.ScrapPoint);
                } 
            }
